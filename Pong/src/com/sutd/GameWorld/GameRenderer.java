...
public class GameRenderer {
	...
	private TextureRegion octopusSmile, fishCake, salmonSushi, riceCracker,
			watermelon, kiwi, orange, wait_screen, splash_screen;
		...
		if (state.getStatus() == 0) // Waiting for client!
		{
			...
			if (countDown == 0) {
				...
				if (countDown > 1) {
					...
					AssetLoader.font.draw(batcher, "GO !", 2 * d.width / 5,
							d.height / 2 - 20);
				}
			}
	...
	private void drawRiceCracker(int centerX, int centerY) {
		...
		// The octopus needs transparency, so we enable that.
		batcher.enableBlending();
		batcher.draw(riceCracker, centerX - radius, centerY - radius,
				2 * radius, 2 * radius);
	}
	...
	private void drawSalmonSushi(int centerX, int centerY) {
		...
		// The octopus needs transparency, so we enable that.
		batcher.enableBlending();
		batcher.draw(salmonSushi, centerX - radius, centerY - radius,
				2 * radius, 2 * radius);
	}
	...
	private void drawFishCake(int centerX, int centerY) {
		...
		// The octopus needs transparency, so we enable that.
		batcher.enableBlending();
		batcher.draw(fishCake, centerX - radius, centerY - radius, 2 * radius,
				2 * radius);
	}

	// for draw function, the x and y parameters indicates the bottom left
	// corner
	private void drawOctopus(int centerX, int centerY) {
		...
		// The octopus needs transparency, so we enable that.
		batcher.enableBlending();
		batcher.draw(octopusSmile, centerX - radius, centerY - radius,
				2 * radius, 2 * radius);
	}
	...
	private void drawTwoOctopus(int centerX, int centerY, float runTime) {
		...
		batcher.draw(AssetLoader.octopusAnimation.getKeyFrame(runTime), centerX
				- radius, centerY - radius, 2 * radius, 2 * radius);
	}
	...
	private void drawPaddle(int centerX, int centerY) {
		...
		shapeRenderer.rect(centerX - width / 2, centerY - height / 2, width,
				height);
	}
}